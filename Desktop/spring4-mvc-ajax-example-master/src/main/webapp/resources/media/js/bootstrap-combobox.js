...
!function($) {
	...
	var Combobox = function(element, options) {
		...
		this.shown = false;
		this.selected = false;
		this.mustMatch = this.options.mustMatch;
		this.refresh();
	...
	Combobox.prototype = {
		...
		template : function() {
			if (this.options.bsVersion == '2') {
				...
				return '<div class="combobox-container"> <input type="hidden" /> <div class="input-group" style="width:100%"> <input type="text" autocomplete="off" /> <span class="input-group-addon dropdown-toggle" data-dropdown="dropdown"> <span class="caret" /> <span class="glyphicon glyphicon-remove" /> </span> </div> </div>'
			}
		}
...
//			this.$element.on('blur', $.proxy(this.toggle, this));
		...
		blur : function(e) {
			...
			if (this.mustMatch == 'true') {
				...
				if (!this.selected && val !== '' && this.oldValue != val) {
					this.$target.val(val);
					this.change(e);
				}									
			}
	...
	$.fn.combobox.defaults = {
		bsVersion : '3',
		menu : '<ul class="typeahead typeahead-long dropdown-menu qp-autocomplete-menu"></ul>',
		item : '<li><a href="#"></a></li>'
	};
