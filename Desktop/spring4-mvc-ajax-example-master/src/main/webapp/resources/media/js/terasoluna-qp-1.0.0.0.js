...
(function() {
				...
				source: function( request, response ) {
			      ...
			      select: function( event, ui ) {			    	  
			    	 // alert("select" + ui.item);
			    	  ...
			    	  var optionLabel = $(autocomplete).attr("optionLabel").split(",");
			    	  console.log("proName" + ui.item.proName);
				...
				source: function( request, response ) {
			      ...
			      change: function (event, ui) {
			    	  ...
			    	  if($(autocomplete).attr("mustmatch") == "true" || $(autocomplete).attr("mustmatch") == undefined) {
			    		 if (!ui.item) {
			    			 ...
			    			 if ($(autocomplete).attr("proName") != '') {
			    				 $(autocomplete).attr("proName", ui.item[$(autocomplete).attr("proName")]);
			    			 }
			    			 var displayOption = "";
			    			 var selectedDisplayValue = $(autocomplete).attr("selectedDisplayValue");
			    			 var optionLabel = $(autocomplete).attr("optionLabel").split(",");
			    			 if (selectedDisplayValue != undefined && selectedDisplayValue != '') {
					    		  displayOption = ui.item[selectedDisplayValue];
					    	 } else {
								  $(optionLabel).each(function(i){
									   displayOption += ui.item[optionLabel[i]] + " - ";
								  });
			      ...
			      select: function( event, ui ) {			    	  
			    	 // alert("select" + ui.item);
			    	  ...
			    	  $(autocomplete).val(displayOption);
			    	  if(($('#previousLabelWhSta').val() == "") && ($(autocomplete).attr('name').indexOf('StockAllocationInformation.wh_id') > 0)){			    		  
			    		  $('#previousLabelWhSta').val(displayOption);
			    	  }
			    	  if(($('#previousLabelBranchSta').val() == "") && ($(autocomplete).attr('name').indexOf('StockAllocationInformation.branch_id') > 0)){
			    		  $('#previousLabelBranchSta').val(displayOption);
			    	  }
					  if(($('#previousLabelWhGdr').val() == "") && ($(autocomplete).attr('name').indexOf('areaGDRC_goods_receipt_hdr.wh_name') >= 0)){			    		  
			    		  $('#previousLabelWhGdr').val(displayOption);
			    	  }
			    	  if(($('#previousLabelBranchGdr').val() == "") && ($(autocomplete).attr('name').indexOf('areaGDRC_goods_receipt_hdr.branch_name') >= 0)){
			    		  $('#previousLabelBranchGdr').val(displayOption);
			    	  }

					    if(($('#previousLabelWhGdr').val() == "") && ($(autocomplete).attr('name').indexOf('areaGDRC_goods_receipt_hdr_931495815.wh_id') >= 0)){			    		  
			    		  $('#previousLabelWhGdr').val(displayOption);
			    	  }
			    	  if(($('#previousLabelBranchGdr').val() == "") && ($(autocomplete).attr('name').indexOf('areaGDRC_goods_receipt_hdr_931495815.branch_id') >= 0)){
			    		  $('#previousLabelBranchGdr').val(displayOption);
			    	  }
		        	  if (ui.item != null) {
		        		  var optionValue = $(autocomplete).attr("optionValue");
	...
	$.qp.getRangeNumber = function(thiz, vMin, vMax) {
		var result = {};
		
		if ($(thiz).attr('qp-min') != undefined && $(thiz).attr('qp-min') != null && $(thiz).attr('qp-min').length > 0) {
			result['vMin'] = $(thiz).attr('qp-min');
		} else {
			result['vMin'] =vMin;
		}
		
		if ($(thiz).attr('qp-max') != undefined && $(thiz).attr('qp-max') != null && $(thiz).attr('qp-max').length > 0) {
			result['vMax'] = $(thiz).attr('qp-max');
		} else {
			result['vMax'] =vMax;
		}
	...
	$.qp.formatBigint = function(thiz) {
		...
		$(thiz).each(function(){
			if ($(this) != undefined && $(this).val() != undefined) {
				//var minMax = $.qp.getRangeNumber(this, "-9223372036854775808", "9223372036854775807");
	...
	$.qp.formatSmallint = function(thiz) {
		$(thiz).each(function(){
			if ($(this) != undefined && $(this).val() != undefined) {
				//var minMax = $.qp.getRangeNumber(this, "-32768", "32767");
				$(this).autoNumeric('init', {
					aSep : ',',
					dGroup : '3',
					aPad : 'false',
	...
	$.qp.formatSerial = function(thiz) {
		$(thiz).each(function(){
			if ($(this) != undefined && $(this).val() != undefined) {
				//var minMax = $.qp.getRangeNumber(this, "0", "2147483647");
				$(this).autoNumeric('init', {
					aSep : ',',
					dGroup : '3',
					aPad : 'false',
	...
	$.qp.formatBSerial = function(thiz) {
		$(thiz).each(function(){
			if ($(this) != undefined && $(this).val() != undefined) {
				//var minMax = $.qp.getRangeNumber(this, "0", "9223372036854775807");
				$(this).autoNumeric('init', {
					aSep : ',',
					dGroup : '3',
					aPad : 'false',
	...
	$.qp.formatInteger = function(thiz) {
		$(thiz).each(function(){
			if ($(this) != undefined && $(this).val() != undefined) {
				//var minMax = $.qp.getRangeNumber(this, "-2147483648", "2147483647");
				$(this).autoNumeric('init', {
					aSep : ',',
					dGroup : '3',
					aPad : 'false',
	...
	$.qp.formatFloat = function(thiz) {
		$(thiz).each(function(){
			if ($(this) != undefined && $(this).val() != undefined) {
				var minMax = $.qp.getRangeNumber(this, "-9999999999999999999999999999.9999999999", "9999999999999999999999999999.9999999999");
				$(this).autoNumeric('init', {
					aSep : ',',
					dGroup : '3',
					aPad : 'false',
					vMin : "-9999999999999999999999999999.9999999999",
					vMax : "9999999999999999999999999999.9999999999"
				});
			}
		});
	};
	...
	$.qp.formatPercentage = function(thiz) {
		$(thiz).each(function(){
			if($(this) != undefined && $(this).val() != undefined) {
				//var minMax = $.qp.getRangeNumber(this, "0", "1000");
				$(this).autoNumeric('init', {
					aSep : ',',
					dGroup : '3',
					aPad : 'false',
	...
	$.qp.formatPercentageDecimal = function(thiz) {
		$(thiz).each(function(){
			if($(this) != undefined && $(this).val() != undefined) {
				//var minMax = $.qp.getRangeNumber(this, "0", "1000.00");
				$(this).autoNumeric('init', {
					aSep : ',',
					dGroup : '3',
					aPad : 'false',
		...
		$(obj).click(function (e) {
			...
			if(passPrecheck){
				...
				if(onselectConfirm != undefined && typeof window[onselectConfirm] == "function"){
					// - 20150612 - change to run on firefox
	...
	$.qp.addRowJSByLink = function(link) {
		...
		$(obj).find("input[type!='radio'][type!='checkbox']").each(function() {
  			 $(this).attr("value","");
  		});
		...
		$(obj).find("input[type='radio'],input[type='checkbox']").each(function() {
 			 if ($(this).next().attr("type") == "hidden") {
 				$(this).next().attr("value", "on");
 			 }
 		});	
		
		//
		var arrRadioChecked = [];
		$(table).find("tbody tr:first").find("input[type='radio']:checked").each(function(){
			arrRadioChecked.push({
					id: $(this).attr("id"),
		...
		// :re-checked and re-name [id] follow [name]
		if(arrRadioChecked.length > 0 ) {
			reCheckedRadio(arrRadioChecked, $(table).find("tbody tr:first"));
		}
		$table.find(">tbody>tr").each(function() {
			$(this).find("[name*='['][name*='].']").each(function() {
				var strName = $(this).attr("name").replace(/\[|\]/g,"");
				$(this).attr("id",strName);
			});
		});
	};
	
	$.qp.addRowJSByLinkNotCheck = function(link) {
		var table = $(link).closest("div").prev("table");
		var obj = $("#" + $(table).attr("id") + "-template").tmpl();
...
//  set condition name (for prototype)
...
$.qp.addMultiRowJSByLink = function(link) {
	...
	reIdMultiRow(table,numberOfRowspan);
};
...
$.qp.reCalIndexMultiRow = function(table) {
	...
	var index = 1;
	$table.find(">tbody>tr").each(function(i) {
		if ($(this).find('td:first[rowspan][rowspan != 0][rowspan != 1]').length > 0) {
...
$.qp.reArrayIndexMultiRow = function(table,numberOfRowspan) {
	...
	$table.find(">tbody td[rowspan="+numberOfRowspan+"][class^='qp-output-fixlength']").each(function(i) {
		...
		for(var j = indexTrFirst+1; j < indexTrFirst + parseInt(numberOfRowspan); j++) {
...
//
function reCheckedRadio(arrRadioChecked,trFirst) {
	for(var i = 0; i < arrRadioChecked.length; i++) {
		$(trFirst).find("[id='"+arrRadioChecked[i].id+"'][value='"+arrRadioChecked[i].value+"']").prop("checked", true);
	}
};

function reIdMultiRow(table,numberOfRowspan) {
	$table=$(table);
	var index = 0;
	var preIndex;
	$table.find(">tbody td[rowspan="+numberOfRowspan+"][class^='qp-output-fixlength']").each(function(i) {
		...
		for(var j = indexTrFirst+1; j < indexTrFirst + numberOfRowspan; j++) {
			var arrTr = $table.find(">tbody tr").get(j);
...
function showConfirm(url,obj,messageConfirmString,requestMedthod) {
	
	//Unbind event submit on confirm of initial
	$(obj).unbind();
	
	var form = $(obj).closest("form").attr("action",url);
	var messageConfirm = messageConfirmString;
	var onselectConfirm = undefined;
	...
	$(obj).closest("div[class='qp-root']").prev("div").find("div[class$='qp-model-body']").html(messageConfirm);
	$("#fcomConfirmDialog").modal('show');
	...
	$("#fcomConfirmBtnYes").on("click", function(e) {
		if($(obj).attr("attrpopup") != undefined && $(obj).attr("attrpopup") != null && $(obj).attr("attrpopup") == 'true') {
			return;
		}
		if(onselectConfirm != undefined && typeof(onselectConfirm) == "function"){
			var result = window[onselectConfirm](event);
